...
class AssessmentEngine:
    ...
    def _combine_heuristic_and_ml_scores(
        ...
        if ml_confidence is not None:
            combined_confidence = (heuristic_confidence * heuristic_weight) + (ml_confidence * ml_weight)
        else:
    ...
    async def assess(self, assessment_input: AssessmentInput) -> AssessmentResult:
        ...
        for path in assessment_input.paths_to_evaluate:
            ...
            if path_score.metrics:
                avg_confidence = sum(m.confidence for m in path_score.metrics) / len(path_score.metrics)
    ...
    def _identify_micro_motives(
        ...
        if path == PathType.TECHNICAL:
            ...
            if any(word in text_lower for word in ['algorithm', 'optimize', 'efficient', 'complexity']):
            ...
            if 'optimize' in text_lower or 'performance' in text_lower:
                motives.append(
                    MicroMotive(
                        ...
                        evidence=self._generate_motive_evidence(submission_text, MotiveType.EFFICIENCY),
                        path_alignment=path,
                    )
                )
            ...
            if collab_indicators:
                motives.append(
                    MicroMotive(
                        ...
                        evidence=self._generate_motive_evidence(submission_text, MotiveType.COLLABORATION),
                        path_alignment=path,
                    )
                )
            ...
            if exploration_indicators:
                motives.append(
                    MicroMotive(
                        ...
                        evidence=self._generate_motive_evidence(submission_text, MotiveType.EXPLORATION),
                        path_alignment=path,
                    )
                )
    ...
    def _analyze_readability(self, text: str) -> float:
        ...
        lines = text.split('\n')
        meaningful_names = sum(1 for line in lines if any(
            word in line.lower() for word in ['name', 'value', 'result', 'data', 'item']
        ))
