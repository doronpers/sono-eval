...
**Error**: `bandit...................................................................Failed` with B104, B404, B603, or B615 issues

**Fix**:
- **B104 (0.0.0.0 bind)**: Add `# nosec B104` comment if intentional for Docker/local dev
- **B404/B603 (subprocess)**: Exclude `scripts/` directory in bandit config (security tools intentionally use subprocess)
- **B615 (HuggingFace)**: Add `# nosec B615` if model versioning is handled elsewhere
- **Scripts directory**: Exclude from bandit checks in `pyproject.toml` and `.pre-commit-config.yaml`

**Configuration**:
- `pyproject.toml`: `exclude_dirs = ["tests", "venv", "scripts"]`
- `.pre-commit-config.yaml`: `exclude: ^(tests/|scripts/)` and use `-ll` flag for severity filtering
...
- `AGENTS.md` - Detailed linting protocols
